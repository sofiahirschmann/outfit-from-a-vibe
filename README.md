...
- `scripts/harvest.mjs` crawls that sitemap at ~1.5s per request (above the declared crawl-delay),
  using ordinary browser headers. Each product page embeds a structured `initialState` JSON blob
  (title, brand, retail `msrpValue`, category facets, color hex, Scene7 image URLs, review count);
  no HTML scraping heuristics, and the disallowed `/api` routes are never touched. The committed
  catalog is the **5,000 most-reviewed pieces** (balanced across slots so the stylist gets a full
  closet), selected from a full ~27,000-item crawl — `--full` collects every product's review
  count, then `--select-top 5000` keeps the best.
...
- **Embeddings: local CLIP** (`@huggingface/transformers`, `Xenova/clip-vit-base-patch32`).
  ...
  No inference API, no keys, no vector DB; 5,000 × 512 floats fit comfortably in memory.
...
```bash
npm run harvest -- --full            # crawl every product + review count → catalog-full-raw.jsonl
npm run harvest -- --select-top 5000 # keep the 5,000 most-reviewed (slot-balanced), LLM-tag them
...
The full crawl is a one-time ~12–15h job at the polite crawl rate; it's resumable. For a quick
refresh of a smaller sample instead, `npm run harvest -- --limit N` uses the older slot-quota path.
...
data/catalog.json          5,000 most-reviewed items, tagged (committed)
