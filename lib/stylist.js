...
// Greedy top-N with a disjointness requirement: outfit 2 shares no items with
// outfit 1, and so on. Requirement loosens only when the pool genuinely can't
// supply disjoint looks.
function pickDistinct(combos, count) {
  ...
  for (let allowShared = 0; allowShared <= 2 && picked.length < count; allowShared++) {
    for (const combo of combos) {
      ...
      if (overlap <= allowShared && !picked.includes(combo)) picked.push(combo);
    }
  }
  return picked;
}
